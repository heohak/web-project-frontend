...
const MessageBox: React.FC<MessageBoxProps> = ({ accountId }) => {
    const [messageBoxData, setMessageBoxData] = useState({
        // We'll need to implement the actual fetch logic, but this is the data we'll need for these message boxes.
        sender: 'John Doe',
    ...
    useEffect(() => {
        const fetchMatchDetails = async () => {
            try {
                ...
                setMessageBoxData({
                  sender: matchData.sender,
                  content: truncatedContent,
                  picture: matchData.picture,
                });
            } catch (error) {
      ...
      <LinkButton to='/' className={`border-fuchsia-300 border border-t-0 flex w-96 rounded-none bg-fuchsia-200 hover:bg-fuchsia-300`}>
        <div className='flex items-center'>
            <div className='bg-fuchsia-600 rounded-full w-10 h-10 flex items-center justify-center'>
                <img src={messageBoxData.picture} />
            </div>
            <div className='ml-4'>
                <p className='text-fuchsia-600'>{messageBoxData.sender}</p>
                <p className='text-fuchsia-500 text-m'>{messageBoxData.content}</p>
            </div>
        </div>
